    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-transparent">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
        <nav className="hidden md:flex space-x-6">
          <Link to="/" className="text-sm font-medium text-white hover:text-[#f5e1d5] transition-colors">Início</Link>
          <Link to="/legado" className="text-sm font-medium text-white hover:text-[#f5e1d5] transition-colors">Legado</Link>
          {isAdmin && (
            <Link to="/admin" className="text-sm font-medium text-white hover:text-[#f5e1d5] transition-colors">
              Admin
            </Link>
          )}
        </nav>
        <Link to="/" aria-label="Belizze" className="flex items-center px-2 py-1 rounded-md bg-white/10">
          <BelizzeSimpleLogo />
        </Link>
        ...
        {mobileMenuOpen && (
          <div className="absolute top-full left-0 right-0 bg-gradient-to-b from-black/90 to-black/80 backdrop-blur-lg mobile-menu">
            <div className="py-4 px-6 flex flex-col space-y-3">
              <Link to="/" className="text-white font-medium py-2 px-3 rounded-md hover:bg-white/10 transition-colors">
                Início
              </Link>
              <Link to="/legado" className="text-white font-medium py-2 px-3 rounded-md hover:bg-white/10 transition-colors">
                Legado
              </Link>
              {isAdmin && (
                <Link to="/admin" className="text-white font-medium py-2 px-3 rounded-md hover:bg-white/10 transition-colors">
                  Admin
                </Link>
              )}
              <a 
